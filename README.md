...
Image enhancements were chosen that could be coded simply in a Jupyter notebook using Python code. Working with code allowed for more control of how enhancements were applied and made it so that the same enhancement could be applied to each image simultaneously (which is difficult in programs like Photoshop). I developed an [Image Enhancement Jupyter notebook](https://github.com/yourusername/JCURA_PyLC_2024/blob/main/JCURA2024_ImageEnhancementCode.ipynb)
